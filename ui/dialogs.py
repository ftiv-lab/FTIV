...
class MarkdownPastePreviewDialog(BaseTranslatableDialog):
    """マークダウンペーストのプレビューダイアログ。

    貼り付けるテキストをパースし、結果をツリー表示して確認できる。
    """
    ...
    def __init__(
        ...
        """MarkdownPastePreviewDialog を初期化する。
        ...
        self.stats_label.setStyleSheet("font-weight: bold; padding: 8px; background: #f0f0f0; border-radius: 4px;")
        ...
        self.tree.setHeaderLabels([tr("label_node_text"), tr("label_node_level")])
        self.tree.setColumnWidth(0, 400)
        layout.addWidget(self.tree)

        # 元テキスト表示（折りたたみ可能）
        self.text_group = QGroupBox(tr("grp_original_text"))
        self.text_group.setCheckable(True)
        self.text_group.setChecked(False)
        ...
        self.text_group.toggled.connect(self._toggle_text_visibility)
    ...
    def _toggle_text_visibility(self, checked: bool) -> None:
        """元テキスト表示の折りたたみ切り替え。"""
        self.original_text_edit.setVisible(checked)
    ...
    def _parse_and_display(self) -> None:
        ...
        stats = self._parse_result["stats"]
        nodes = self._parse_result["nodes"]

        # 統計情報表示
        if stats["node_count"] == 0:
            self.stats_label.setText(
                tr("msg_markdown_parse_failed").format(
                    total=stats["total_lines"],
                    skipped=stats["skipped_lines"],
                )
            )
            self.stats_label.setStyleSheet("font-weight: bold; padding: 8px; background: #ffcccc; border-radius: 4px;")
            self.button_box.button(QDialogButtonBox.Ok).setEnabled(False)
        else:
            self.stats_label.setText(
                tr("msg_markdown_parse_success").format(
                    nodes=stats["node_count"],
                    total=stats["total_lines"],
                    parsed=stats["parsed_lines"],
                )
            )
            self.stats_label.setStyleSheet("font-weight: bold; padding: 8px; background: #ccffcc; border-radius: 4px;")

        # ツリー表示
        self.tree.clear()
        self._add_nodes_to_tree(nodes, None, 0)
        self.tree.expandAll()

    def _add_nodes_to_tree(self, nodes: list, parent_item: Optional[Any], level: int) -> None:
        """再帰的にノードをツリーに追加する。"""
        ...
        for node_data in nodes:
            ...
            if parent_item is None:
                item = QTreeWidgetItem(self.tree, [text, str(level)])
            else:
                item = QTreeWidgetItem(parent_item, [text, str(level)])

            if children:
                self._add_nodes_to_tree(children, item, level + 1)
    ...
    def refresh_ui_text(self) -> None:
        ...
        if hasattr(self, "text_group"):
            self.text_group.setTitle(tr("grp_original_text"))
